...
void calculate_instantaneous_CCDF()
{
    ...
    TrellisShaping TS(CONSTELLATION_SIZE, TSMetricType::Clipping, TSMappingType(TS_MAPPING_TYPE));

    ofstream file(join("./Result/CBTS/CCDF_Norm_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ...
    for (int i = 0; i < NUMBER_OF_TRIALS; i++) {
        ...
        TS.make_reference_symbols(transmittedCodes, SCALING_FACTOR, CLIPPING_RATIO);
...
void calculate_average_power_reduction_capability()
{
    ...
    ofstream file(join("./Result/CBTS/APR_Capa_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ...
    for (int i = 0; i < NUMBER_OF_TRIALS; i++) {
        ...
        capability += dB(1.0 / get_average_power(transmittedSymbols)) / (double)(NUMBER_OF_TRIALS);
    }

    cout << setw(5) << capability << "[dB]" << "\n";
...
void calculate_clipping_ratio_character()
{
    ...
    OFDM OFDM(NUMBER_OF_SUBCARRIERS, 0, OVERSAMPLING_FACTOR);
    TrellisShaping TS(CONSTELLATION_SIZE, TSMetricType::Clipping, TSMappingType(TS_MAPPING_TYPE));
    ofstream file(join("./Result/CBTS/Clipping_Chara_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ...
    for (double  clippingRatio = 2.0; clippingRatio > 0; clippingRatio -= 0.1) {
        ...
        for (int i = 0; i < NUMBER_OF_TRIALS; i++) {
            ...
            TS.make_reference_symbols(transmittedCodes, SCALING_FACTOR, clippingRatio);
...
void calculate_uncoded_BER_over_AWGN()
{
    ...
    FERCounter FERCounter((NUMBER_OF_BITS - 1) * NUMBER_OF_SUBCARRIERS);
    vec EbN0s = linspace_fixed_step(15.0, 25.0);
    vec SNRs = TS.get_rate() * NUMBER_OF_BITS * inv_dB(EbN0s);
    ofstream file1(join("./Result/CBTS/BER_Uncod_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/SER_Uncod_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file3(join("./Result/CBTS/FER_Uncod_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ...
    for (int i = 0, size = EbN0s.size(); i < size; i++) {
        ...
        file3 << dB(SNRs[i]) << " " << FERCounter.get_errorrate() << endl;
...
void calculate_uncoded_BER_over_Rayleigh()
{
    ...
    vec SNRs = TS.get_rate() * NUMBER_OF_BITS * inv_dB(EbN0s);
    ofstream file1(join("./Result/CBTS/BER_Uncod_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/FER_Uncod_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
...
void calculate_turbo_coded_BER_over_AWGN()
{
    ...
    ofstream file1(join("./Result/CBTS/BER_Turbo_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/FER_Turbo_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
...
void calculate_turbo_coded_BER_over_Rayleigh()
{
    ...
    vec SNRs = TS.get_rate() * turboCode.get_rate() * NUMBER_OF_BITS * inv_dB(EbN0s);
    ofstream file1(join("./Result/CBTS/BER_Turbo_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/FER_Turbo_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
...
void calculate_theoretical_BER_over_AWGN()
{
    ...
    double BER;
    double SER;
    ofstream file1(join("./Result/CBTS/BER_Theory_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/SER_Theory_AWGN_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
...
void calculate_theoretical_BER_over_Rayleigh()
{
    ...
    QAM QAM(CONSTELLATION_SIZE);
    TrellisShaping TS(CONSTELLATION_SIZE, TSMetricType::Clipping, TSMappingType(TS_MAPPING_TYPE));
    Histogram MPamSymbolHistogram(0, sqrt(CONSTELLATION_SIZE) - 1, sqrt(CONSTELLATION_SIZE));
    Histogram MQamSymbolHistogram(0, CONSTELLATION_SIZE - 1, CONSTELLATION_SIZE);
    vec SNRs = inv_dB(linspace_fixed_step(10.0, 40.0));
    double capability = 0.0;
    vec MPamSymbolDistribution, MQamSymbolDistribution;
    double occurrenceProbability;
    double numberOfMSBError = 0.0, numberOfLSBError = 0.0, numberOfBitError;
    double BER;
    double SER;
    ofstream file1(join("./Result/CBTS/BER_Theory_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ofstream file2(join("./Result/CBTS/SER_Theory_Rayleigh_", CONSTELLATION_SIZE, "-QAM_", NUMBER_OF_SUBCARRIERS, "-Sub", "_β-", SCALING_FACTOR, ".dat"), ios::out);
    ...
    for (int i = 0; i < NUMBER_OF_TRIALS; i++) {
        ...
        MPamSymbolHistogram.read_number(QAM.demodulate_real(transmittedSymbols));
        MQamSymbolHistogram.read_number(QAM.demodulate(transmittedSymbols));
    }

    // Calculate the symbol occurrence probability.
    MPamSymbolDistribution = MPamSymbolHistogram.get_discrete_histogram();

    occurrenceProbability = MPamSymbolDistribution[0];
    for (int i = 1; i < MPamSymbolDistribution.size() - 1; i++) {
        occurrenceProbability += 2.0 * MPamSymbolDistribution[i];
    }
    occurrenceProbability += MPamSymbolDistribution[MPamSymbolDistribution.size() - 1];

    // Calucalate the number of bit errors.
    MQamSymbolDistribution = MQamSymbolHistogram.get_discrete_histogram();

    for (int LSB = 0; LSB < 2; LSB++) {
        for (int MSB = 0; MSB < NUMBER_OF_BITS - 2; MSB++) {
            int index1 = LSB + MSB * pow2i(2);
            ivec adjacentIndexes = QAM.demodulate(QAM.get_adjacent_symbols(index1));
            helper::dump(index1, adjacentIndexes);

            for (int i = 0; i < 2; i++) {
                int index2 = i + MSB * pow2i(2);

                if (contains(index2, adjacentIndexes)) {
                    helper::dump("a");
                    numberOfLSBError += MQamSymbolDistribution[index1] / adjacentIndexes.size()
                        * hamming_distance(dec2bin(NUMBER_OF_BITS, index1), dec2bin(NUMBER_OF_BITS, index2));
                }
            }
            for (int i = 0; i < NUMBER_OF_BITS - 2; i++) {
                int index2 = LSB + i * pow2i(2);

                if (contains(index2, adjacentIndexes)) {
                    helper::dump("b");
                    numberOfMSBError += MQamSymbolDistribution[index1] / adjacentIndexes.size()
                        * hamming_distance(dec2bin(NUMBER_OF_BITS, index1), dec2bin(NUMBER_OF_BITS, index2));
                }
            }
        }
    }
    helper::dump(numberOfMSBError, numberOfLSBError);
    numberOfBitError = numberOfMSBError + numberOfLSBError * 2.5;
...
int main(const int argc, const char* argv[])
{
    ...
    } if (arguments[1] == "average") {
        calculate_average_power_reduction_capability();
    } else if (arguments[1] == "clipping") {
